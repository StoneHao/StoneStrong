...
$(function () {
    ...
    $('.search_btn').on('tap', function () {
        ...
        var isHave = false;
        var haveIndex = null;
        for (var i = 0; i < arr.length; i++) {//判断是否存在相同的数据
            if (key == arr[i]) {
                isHave = true;
                haveIndex = i;
                break;
        ...
        /*存起来 格式是一个json数组形式的字符串 再取出的时候转成js数组的数据格式才不会报错*/
...
var getHistoryData = function () {
    ...
    var str = localStorage.getItem('leTaoHistory') || '[]';//返回值是一个类似数组形式的字符串
    /*3.转成成js数据*/
    var arr = JSON.parse(str);//将JSON数据转化成JS数组
    /*4.返回js可操作的数组*/
    return arr;
};
